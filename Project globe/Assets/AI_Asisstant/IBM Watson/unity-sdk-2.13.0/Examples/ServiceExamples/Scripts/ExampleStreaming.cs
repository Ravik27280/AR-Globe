        ...
        if (result != null && result.results.Length > 0)
        {
            foreach (var res in result.results)
            {
                foreach (var alt in res.alternatives) // This section outputs the Speech to Text Predicitons.
                {
                    ...
                    {
                        StopRecording();
                        StartCoroutine(ExampleTextToSpeech.I1_HelloVikram());       //When it detect these words it will execute a Coroutine in the Text to Speech script.
                        StartCoroutine(wait(4));
                    }

                    if (text.Contains("history") && State.Contains("Final")) // Bring up design interface
                    {
                        StopRecording();

                        StartCoroutine(ExampleTextToSpeech.I2_History());
                       // Holodeck.gameObject.SetActive(true);                      //Activates game object/s on Command.

                        StartCoroutine(wait(4));
                    }

                    if (text.Contains("show") && State.Contains("Final")) // open up project edith prototype
                    {
                        StopRecording();
                    ...
                    if ((text.Contains("missions")) && State.Contains("Final")) // pause rotation
                    {
                        StopRecording();
                        StartCoroutine(ExampleTextToSpeech.I4_mission());
                        if (rotate.speed == 10)
                        {
                            rotate.speed = 0;
                        }
                    ...
                    if (text.Contains("upcoming") && State.Contains("Final")) // Jarvis bring up status
                    {
                        StopRecording();
                        StartCoroutine(ExampleTextToSpeech.I5_Upcoming());
                        StartCoroutine(wait(7));
                    }

                    if (text.Contains("who") && State.Contains("Final")) // Integrate deadshot module
                    {
                        StopRecording();
                        StartCoroutine(ExampleTextToSpeech.I6_Who());
                        //Deadshot.SetActive(true);
                        //StartCoroutine(MoveTarget.MoveTargetToDestinations(Deadshot));
                        StartCoroutine(wait(5));
                    }

                    //if (text.Contains("continue") && State.Contains("Final")) 
                    //{
                    //    if (rotate.speed == 0)
                    //    {
                    //        rotate.speed = 10;
